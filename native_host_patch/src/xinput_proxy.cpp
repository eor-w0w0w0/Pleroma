// Minimal no-CRT xinput proxy for Darktide native patching.
...
static constexpr DWORD ERROR_DEVICE_NOT_CONNECTED = 1167;
static constexpr DWORD LOAD_LIBRARY_SEARCH_SYSTEM32 = 0x00000800;
...
using XInputGetStateFn = DWORD (*)(DWORD, void *);
using XInputSetStateFn = DWORD (*)(DWORD, void *);

namespace proxy {

struct State {
    HMODULE self_module;
    HMODULE real_xinput_module;
    ...
    XInputSetStateFn xinput_set_state;
    int initialized;
};
...
static void copy_wide_string(WCHAR *dst, const WCHAR *src)
{
    size_t i = 0;
...
static HMODULE load_real_xinput_module()
{
    ...
    for (size_t i = 0; i < sizeof(CANDIDATES) / sizeof(CANDIDATES[0]); ++i) {
        HMODULE module = g_state.load_library_exw(CANDIDATES[i], nullptr, LOAD_LIBRARY_SEARCH_SYSTEM32);
...
static void initialize_runtime()
{
    ...
    write_log_line("xinput proxy initialized");
    ...
    g_state.real_xinput_module = load_real_xinput_module();

    if (!g_state.real_xinput_module) {
        write_log_line("system xinput failed to load");
        return;
    }

    g_state.xinput_get_state = reinterpret_cast<XInputGetStateFn>(get_proc_address(g_state.real_xinput_module, "XInputGetState"));
    g_state.xinput_set_state = reinterpret_cast<XInputSetStateFn>(get_proc_address(g_state.real_xinput_module, "XInputSetState"));

    write_log_line(g_state.xinput_get_state && g_state.xinput_set_state ? "system xinput exports resolved" : "system xinput exports missing");

}

} // namespace proxy

extern "C" DLL_EXPORT DWORD XInputGetState(DWORD user_index, void *state)
{
    proxy::initialize_runtime();

    if (!proxy::g_state.xinput_get_state) {
        return ERROR_DEVICE_NOT_CONNECTED;
    }

    return proxy::g_state.xinput_get_state(user_index, state);
}

extern "C" DLL_EXPORT DWORD XInputSetState(DWORD user_index, void *state)
{
    proxy::initialize_runtime();

    if (!proxy::g_state.xinput_set_state) {
        return ERROR_DEVICE_NOT_CONNECTED;
    }

    return proxy::g_state.xinput_set_state(user_index, state);
}

extern "C" DLL_EXPORT int SoloPlayXInputProxy_IsReady()
{
    proxy::initialize_runtime();
    return proxy::g_state.real_xinput_module != nullptr && proxy::g_state.xinput_get_state != nullptr && proxy::g_state.xinput_set_state != nullptr;
}

extern "C" DLL_EXPORT int SoloPlayXInputProxy_LoadPatch()
{
    proxy::initialize_runtime();
...
extern "C" DLL_EXPORT int SoloPlayXInputProxy_PatchLoaded()
{
    proxy::initialize_runtime();
    return proxy::g_state.patch_module != nullptr;
}
