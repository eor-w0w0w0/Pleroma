...
StepperTemplates.difficulty_stepper = {
	{
		pass_type = "logic",
		value = function (pass, ui_renderer, logic_style, content, position, size)
			local now = Managers.time:time("main")
			local time_ready = true
			if content.enable_time then
				time_ready = now - content.enable_time > 0.3
			end
			if not content.disabled and time_ready then
